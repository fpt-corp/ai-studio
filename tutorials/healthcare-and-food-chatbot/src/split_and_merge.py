...
VAL_SAMPLES = 100
TEST_SAMPLES = 100
OUTPUT_DIR = Path("data/final_data/healthcare_and_vn_food")
...
for subfolder in BASE_DIR.iterdir():
    if not "create_healthcare_conversation" in subfolder.name and not "introduce_vn_food" in subfolder.name:
        continue
    if subfolder.is_dir():
        ...
        if "introduce_vn_food" in subfolder.name:
            VAL_SAMPLES = 10
            TEST_SAMPLES = 10
        else:
            VAL_SAMPLES = 100
